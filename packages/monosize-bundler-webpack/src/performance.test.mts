/**
 * Performance comparison test for single-build vs loop mode.
...
// Skip this test by default - run manually with: yarn test performance.test
describe.skip('Performance Comparison', () => {
  it('compares single-build vs loop mode for 10 fixtures', async () => {
    const { fixtures } = await setupMultipleFixtures(10);
    ...
    console.log(`Speedup:           ${(loopDuration / singleBuildDuration).toFixed(2)}x`);
    console.log(`Time saved:        ${(loopDuration - singleBuildDuration).toFixed(2)}ms`);
    console.log('=====================================\n');

    // Expect single-build to be faster (though this might not always be true for small numbers)
    expect(singleBuildDuration).toBeLessThan(loopDuration * 1.5); // Allow some variance
  }, 120000); // 2 minute timeout

  it('compares single-build vs loop mode for 20 fixtures', async () => {
    const { fixtures } = await setupMultipleFixtures(20);
    ...
    console.log('\n=== Performance Comparison Results (20 fixtures) ===');
    console.log(`Loop mode:         ${loopDuration.toFixed(2)}ms`);
    console.log(`Single-build mode: ${singleBuildDuration.toFixed(2)}ms`);
    console.log(`Speedup:           ${(loopDuration / singleBuildDuration).toFixed(2)}x`);
    console.log(`Time saved:        ${(loopDuration - singleBuildDuration).toFixed(2)}ms`);
    ...
    // Expect single-build to be faster for larger numbers
